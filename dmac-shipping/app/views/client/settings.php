    ...
    try {
        if ($action === 'profile') {
            ...
            if (!preg_match('/^09\d{9}$/', $contact)) {
                header('Location: settings.php?error=contact');
                exit();
            }
            ...
            <div class="settings-card">
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="tel" name="client_contact" value="<?= h($client['client_contact'] ?? '') ?>" required maxlength="11" minlength="11" inputmode="numeric" pattern="09[0-9]{9}" title="Contact number must be exactly 11 digits and start with 09.">
                    </div>

                    <div class="form-group">
                        <label>Email</label>
                        <input type="email" name="client_email" value="<?= h($client['client_email'] ?? '') ?>" required pattern="^[^\s@]+@[^\s@]+\.[^\s@]{2,}$" title="Please enter a valid email address, example: name@gmail.com">
                    </div>
